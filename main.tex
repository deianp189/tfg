...
\begin{itemize}
	\item \textbf{Planificación y arquitectura (abril 2025)}\\
	...
	\item \textbf{Desarrollo intensivo y pivotes técnicos (primera mitad de mayo 2025)}\\
	...
	\item \textbf{Refinamiento, pruebas y validación (segunda mitad de mayo 2025)}\\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{cronologia/1.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{cronologia/2.png}
...
\begin{itemize}
	\item \textbf{Introducción, Motivación y Objetivos}\\
	\hspace*{1em}El documento comienza contextualizando la problemática de la ciberseguridad en PYMEs, explicando la motivación personal y profesional detrás del proyecto, así como los objetivos generales y específicos que se han perseguido durante su desarrollo.
	
	\item \textbf{Fases de la realización y cronograma}\\
	\hspace*{1em}En este apartado se describe cómo se ha organizado y ejecutado el trabajo a lo largo del tiempo, recogiendo las distintas etapas del desarrollo: desde la planificación inicial y el diseño de la arquitectura, pasando por la implementación de los distintos módulos, hasta las fases finales de pruebas, validación y documentación.
	
	\item \textbf{Capítulo 1: Sistemas de detección de intrusos: Snort y frontends}\\
	...
	\item \textbf{Capítulo 2: Diseño e implantación de un frontend para R-Snort}\\
	...
	\item \textbf{Capítulo 3: Caso práctico — utilización del frontend de R-Snort}\\
	...
	\hspace*{1em}A continuación, se presentan y analizan los resultados obtenidos tras la validación del sistema.
...
\begin{table}[hbtp]
	...
	\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}X||>{\centering\arraybackslash}X||>{\centering\arraybackslash}X||>{\centering\arraybackslash}X|}
		...
		\hhline{|====|} % ← redibuja las cuatro reglas sobre el fondo gris
...
Las pruebas de rendimiento realizadas en el propio complemento de TFG han mostrado que, bajo condiciones de tráfico pasivo y también con cargas simuladas extremas, la Raspberry Pi 5 mantiene un uso controlado de CPU, RAM y disco, sin alcanzar límites de saturación que impidan el correcto funcionamiento. Por ejemplo, en la Figura~\ref{fig:raspberry-carga-normal} se observa que, durante el funcionamiento habitual del sistema, la CPU se mantiene en estado inactivo durante la mayor parte del tiempo. Bajo una situación de carga intensa, como se muestra en la Figura~\ref{fig:raspberry-carga-extrema}, se produce un aumento significativo del uso del procesador, especialmente en espacio de kernel, sin llegar a comprometer la estabilidad del sistema. A su vez, la Figura~\ref{fig:raspberry-carga-sistema-extrema} demuestra que la carga media del sistema crece de forma sostenida pero controlada, lo que indica un reparto adecuado de los recursos sin cuellos de botella. \newline
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{ref_complemento/1.1.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{ref_complemento/2.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{ref_complemento/1.png}
	\caption{Carga del sistema de la Raspaberry Pi 5 bajo en condiciones extremas.}
...
\begin{itemize}
	\item \textbf{Inicio de sesión.} Es la pantalla inicial de la aplicación, donde el usuario introduce sus credenciales (\emph{correo electrónico} y \emph{contraseña}) para acceder. Tras hacer clic en \path{Acceder}, el backend valida las credenciales y, de ser correctas, emite un token de autenticación JWT. La apariencia de esta vista es sencilla y centrada, siguiendo las pautas de diseño minimalista.
	
	\item \textbf{Selector de agente.} Una vez autenticado, el usuario puede seleccionar cuál de los agentes disponibles desea monitorizar. En la barra superior de la SPA se muestra un menú desplegable que lista los agentes registrados identificados por nombre o dirección IP, e.j. \path{Central (192.168.1.171)}. Al elegir un agente, el frontend carga los datos correspondientes a ese sensor en las vistas de alertas, reglas y estado. Este mecanismo permite controlar múltiples sensores desde la misma interfaz de forma conveniente.
...
\appendix
\renewcommand{\appendixname}{Anexo}                % Cambia "Apéndice" por "Anexo" (para \appendixpage, si lo usas)
\renewcommand{\chaptername}{Anexo}                 % Cambia el encabezado de capítulo
\renewcommand{\thechapter}{\Roman{chapter}}        % Usa números romanos: I, II, III...
\chapter{Descripción de la plataforma R-Snort}
\label{anexo-descripcion-snort}
...
\thispagestyle{empty}
\newpage
\chapter{Repositorios y guía rápida}
\label{anexo-repo-snort}
...
\section{Repositorio del Agente R-Snort}
...
\begin{itemize}
	...
	\item Para soporte adicional, utiliza el sistema de \emph{issues} o \emph{pull requests} de GitHub, o consulta los enlaces a la documentación técnica que acompañan cada proyecto.
\end{itemize}
...
\thispagestyle{empty}
\newpage
\chapter{Guía para escribir reglas Snort 3}
\label{anexo-reglas-snort}

Este anexo ofrece un \emph{manual de instrucciones} para que cualquier usuario pueda redactar sus propias reglas en Snort 3. Se parte de cero, pero se incluye todo lo esencial para que las reglas resulten funcionales.

\section{Estructura general de una regla}

En Snort 3 una regla se compone de dos partes:

\begin{enumerate}
	\item \textbf{Cabecera (header)} – define \emph{qué} tráfico interesa.
	\item \textbf{Opciones (rule options)} – especifican \emph{por qué} ese tráfico debe generar una alerta.
\end{enumerate}

\begin{lstlisting}[language=bash, caption={Plantilla mínima de regla Snort 3},label={lst:snort3-regla}]
	alert <proto> <src_ip> <src_port> -> <dst_ip> <dst_port> (
	msg:"Texto descriptivo";
	content:"cadena o patrón";
	sid:1000001;
	rev:1;
	)
\end{lstlisting}
...
\begin{itemize}
	\item \textbf{Acción}: \verb|alert|, \verb|log|, \verb|pass|, \verb|drop|, \verb|reject| o \verb|sdrop|.
	\item \textbf{Protocolo}: \verb|tcp|, \verb|udp|, \verb|icmp|, \verb|ip|, \verb|http2|, etc.
	\item \textbf{Direcciones/puertos}: pueden ser IP, rangos (\verb|192.168.1.0/24|), variables (\verb|$HOME_NET|) o \verb|any|.
	\item \textbf{Dirección de flujo}: \verb|->| (unidireccional) o \verb|<>| (bidireccional).
\end{itemize}
...
\begin{enumerate}
	...
	\item Ejecuta la comprobación sintáctica:
	\begin{lstlisting}[language=bash]
		sudo snort -c /etc/snort/snort.lua --dry-run
	\end{lstlisting}
	\item Si no hay errores, reinicia Snort:
	\begin{lstlisting}[language=bash]
